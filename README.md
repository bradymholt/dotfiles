...
- `link.sh` -  If there are files in $HOME that you would like to be moved to the dotfiles repo, you can run this script and pass in the name of the file that resides in $HOME.  This will move the file to the dotfiles directory and create a symlink to it.  Use `--secret` to move the file to the `.secrets` directory instead (e.g. `./link.sh --secret .ssh/id_rsa`).
